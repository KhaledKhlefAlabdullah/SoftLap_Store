  ...
  const fetchProductDetails = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setProductDetails(product_Details.product);
        setImages(product_Details.imgs);
        setSimilarProducts(product_Details.related_products);
        setIsLoading(false);
      }
    } catch (err) {
      console.error(err);
              ...
              <Row className="d-flex align-items-center justify-content-between">
                <Container className="col-lg-4 col-md-6 col-sm-12 p-sm-2">
                  <Container className="imgs-container rounded m-md-2">
                    <img
                      src={SERVER_PATH + product_details.face_image}
                      alt={`Product ${product_details.face_image}`}
                ...
                <Container key={product.id} className="col-lg-3 col-md-4 col-sm-6 mb-4">
                  <Link to={`/product/${product.id}`} className="text-decoration-none">
                    <Container className="product-card my-box-shadow p-2 rounded-lg">
                      <img
                        src={SERVER_PATH + product.face_image}
                        alt={`Product ${product.face_image}`}
